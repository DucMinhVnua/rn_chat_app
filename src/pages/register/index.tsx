            ...
            {Object.values(DATASFIELD(formValue)).map((fieldData, index) => (
               <React.Fragment key={`${index}`}>
                  <InputTextAuthentication
                     ...
                     isPassWord={
                        fieldData.name === DATA_UI.fieldNamePassword ||
                           fieldData.name === DATA_UI.fieldNameConfirmPassword
                           ? true
                           : false
                     }
                  />
   ...
   function renderRegisterButton() {
      ...
      const handleRegister = async () => {
         ...
         async function handleGetAndCheckAccount() {
            const accounts = await baseRealtimeDB.oneTimeRead(
               RoutesNameFBDB.register,
            );

            // if table hasn't in BD
            if (!accounts) {
               return false;
            }
         ...
         async function writeAccoutToDB() {
            return await baseRealtimeDB.settingData(
               RoutesNameFBDB.register,
               BODY_REQUEST(formValue),
            );
         }
      };
